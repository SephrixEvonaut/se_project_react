...
async function getClothingItems() {
  ...
  try {
    let clothingItems = await fetch(`${baseUrl}/items`);
    if (!clothingItems.ok) {
      return Promise.reject(`Error ${clothingItems.status}`);
    }

    let clothingItemsJSON = await clothingItems.json();

    return clothingItemsJSON;
  } catch (error) {
